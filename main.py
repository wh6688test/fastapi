...
@app.get("/group/member/{member_id}/", response_model=List[Group], summary="retrieving the group that contains the member and its rating", response_description="group that contains the member has been returned")
...
async def create_group(grp:Group_In=Body(..., embed=True)):
       result={}
       result['group_id']=uuid4()
       result['group_attribute']=grp.group_attribute

       data=findGroupInJsonByGid(s_output, result['group_id'])
       if  data:
...
def update_member_rating(group_id:str=Query(..., max_length=50), member:Member=Body(...)):

      data1=findGroupInJsonByGid(s_output, group_id)
      logger.info("data1 in member")
      logger.info(data1)
      if not data1 or len(data1) == 0:
         raise HTTPException(status_code=404, detail="group does not exist") 

      result=updateMembers(s_output, group_id, member)
      return result
...
def update_group_attribute(group_id:str = Query(..., max_length=50), attrs:Group_attr=Body(..., embed=True)):
      ...
      if not data1 or len(data1) == 0:
         raise HTTPException(status_code=404, detail="group does not exist") 
      #if not data1 or len(data1) == 0:
          #empty response , no update is needed : just try some variations
      #   return Response(status_code=status.HTTP_200_OK) 
      result=updateAttributes(s_output, group_id, attrs)
      print(result)
      return result
